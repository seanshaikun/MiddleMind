...
from NewsAgg2 import publish as pulish
...
@rt("/")
def get():
    return Nav(Li(('Menu'), Nbsp(),
    Button('News',hx_get="/page1", hx_target="#page-content"), Nbsp(),
    Button('Currated',hx_get='/page2',hx_target="#page-content")),Li(Button('Login',href='/login'))),Titled('The Middle Mind Collective',
    P(),Div(page1(),id='page-content'))
...
def page1():
    ...
    return Div(
        ...
        Grid(
            Div(H2('Border Security'),P(*(Article(s) for s in stories if "Border Security" in s.topic))),
...
@basic_auth
def get(session):    
    return Titled("Dashboard",Button("Logout", hx_post="/logout"),
    Nbsp(),Button("Currated", hx_swap='outerHTML',hx_get="/curated"),P(),Nbsp(),
    Nbsp(),Button("Update", hx_post="/update"),P(),Nbsp(),
    )
@rt('/update')
def post():
    pulish()
